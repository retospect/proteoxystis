...
def parse_commandline():
    ...
    try:
        ...
        pass
    parser.add_argument('--preload', type=str, default=load_model_default, help='preload a model from this model file')
    ...
    parser.add_argument('--predict', type=str, default=None, help='predict a pdb entry by pdb id')
...
def predict(model, metadata, seqs_train, output_train, seqs_test, output_test, pdbid):
    ...
    if pdbid in metadata["pdb_names_test"]:
        ...
        in_data = torch.from_numpy(seqs_test[index]).float().to(find_torch_training_device())
        out_data = torch.from_numpy(output_test[index]).float().to(find_torch_training_device())
    if pdbid in metadata["pdb_names_train"]:
        index = metadata["pdb_names_train"].index(pdbid)
        in_data = torch.from_numpy(seqs_train[index]).float().to(find_torch_training_device())
        out_data = torch.from_numpy(output_train[index]).float().to(find_torch_training_device())
    ...
    for k in all_keys:
        ...
        # print the row
        print("{:19s} {:10.2f} {:10.2f} {}".format(k, actual_value, predicted_value, delta))
