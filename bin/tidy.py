...
with open("training.toml", "wt") as f:
    ...
    for key in tqdm.tqdm(pdb.keys()):
        try:
            ...
            if l < 15:
                raise PdbParseException(
                    f"ERROR 54: [{key}] Sequence length {l} is too short".format(
                        key=key, l=l
                    )
                )
            ...
            if re.search(r"\s", value["sequence"]):
                raise PdbParseException(
                    f"ERROR 56: [{key}] Sequence contains whitespace".format(key=key)
                )
            ...
            if crco is not None:
                ...
                if m is not None:
                    ...
                    try:
                        ...
                        raise PdbParseException(
                            f"ERROR 55: [{key}] Could not parse PH from crystal conditions: {crco}".format(
                                key=key, crco=crco
                            )
                        )
                ...
                if m is not None:
                    ...
                    for c in m.group(1).split(","):
                        ...
                        try:
                            ...
                            raise PdbParseException(
                                f"ERROR 57: [{key}] Could not parse crystallization conditions (doublette error): {crco}".format(
                                    key=key, crco=crco
                                )
                            )
            ...
            for val in values.values():
                if val >= np.finfo(np.half).max:
                    raise PdbParseException(
                        f"ERROR 59: [{key}] Is not interesting, value too large: {crco}".format(
                            key=key, crco=crco
                        )
                    )
                    continue
                if val <= np.finfo(np.half).min:
                    raise PdbParseException(
                        f"ERROR 60: [{key}] Is not interesting, value too small: {crco}".format(
                            key=key, crco=crco
                        )
                    )
                    continue
